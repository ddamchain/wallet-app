...
RCT_EXPORT_METHOD(signTx:(NSString *)sk
  ...
  //          String sign = Account.signTx(sk,targetAddr,longValue,gasLimit ,gasPrice, txType, nonce, dataHex);
// FOUNDATION_EXPORT NSString* _Nonnull AccountSignTx(NSString* _Nullable sk, NSString* _Nullable targetAddr, int64_t value, int64_t gasLimit, int64_t gasPrice, long p5, int64_t nonce, NSString* _Nullable dataHex);
  
  NSString *sign = AccountSignTx(sk, targetAddr, longValue, gasLimit, gasPrice, txType, nonce, dataHex);
